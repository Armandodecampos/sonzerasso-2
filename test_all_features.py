...
def test_all_new_features(page: Page):
    ...
    0. A funcionalidade de tocar música aleatória (simulando o arranque).
    ...
    # Etapa 0: Verificar se a música toca (simulando o arranque)
    print("Verificando a funcionalidade de tocar música ao carregar...")
    music_player = page.locator("#music-player")
    player_title = page.locator("#player-song-title")

    # Simplesmente mostra a UI e chama a função de tocar música,
    # que é o comportamento esperado no final do fluxo de arranque.
    page.evaluate("() => toggleUI(true)")
    page.evaluate("() => window.playRandomSong()")

    # A música deve começar a tocar
    expect(music_player).to_have_class(re.compile(r"player-visible"), timeout=15000)
    expect(player_title).not_to_have_text("Nome da Música", timeout=15000)
    print("✅ Leitor de música visível e a tocar no arranque.")

    # O resto do teste continua...
